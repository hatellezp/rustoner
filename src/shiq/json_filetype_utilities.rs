...
// use crate::shiq::node::Mod;
use crate::shiq::tbox::TB_SHIQ;
use crate::shiq::tbox_item::TBI_SHIQ;
use crate::kb::types::DLType;
// use serde::Deserializer;
use crate::shiq::string_formatter::{node_to_string, string_to_node};
                ...
                match (lside_result, rside_result) {
                    ...
                    (Ok(lside), Ok(rside)) => {
                        ...
                        match (&lside, &rside) {
                            ...
                            (Ok(ls), Ok(rs)) => {
                                let level: usize = 0;
                                let new_tbi_op = TBI_SHIQ::new(ls.clone(), rs.clone(), level);
...
pub fn parse_value_abi(_value: &Value, _symbols: Vec<&Node_SHIQ>) -> Option<ABI_SHIQ> {
    Option::None
}
                            ...
                            if map.contains_key("tbox") {
                                ...
                                match value_array {
                                    Value::Array(vec_of_values) => {
                                        ...
                                        let mut tbi_result: io::Result<TBI_SHIQ>;
...
fn node_to_value(node: &Node_SHIQ, symbols: &SymbolDict) -> Option<Value> {
...
fn tbi_to_value(tbi: &TBI_SHIQ, symbols: &SymbolDict) -> Option<Value> {
