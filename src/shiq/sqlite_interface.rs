...
use crate::shiq::native_filetype_utilities::find_bound_of_symbols;
use crate::shiq::node::Node_SHIQ;
use crate::shiq::sqlite_structs::{AboxQItemCDb, AboxQItemRDb, NodeDb, SymbolDb, TableDb, TboxItemDb};
use crate::shiq::string_formatter::{node_to_string, string_to_abiq, string_to_tbi};
...
pub fn add_node_to_db(
    ...
    match node_string_op {
        ...
        Some(node_string) => {
            ...
            // add the childs too
            if !is_base {
                let child = Node_SHIQ::child_old(Some(&node)).unwrap();
