...
use crate::shiq::rule::{
    shiq_abox_rule_one, shiq_abox_rule_three, shiq_abox_rule_two,
};
...
impl ABox for ABQ_SHIQ {
    type AbiItem = ABIQ_SHIQ;
    ...
    fn add(&mut self, abi: ABIQ_SHIQ) -> bool {
        /*
    ...
    fn items(&self) -> &Vec<ABIQ_SHIQ> {
        &self.items
    }

    fn get(&self, index: usize) -> Option<&ABIQ_SHIQ> {
    ...
    fn contains(&self, abi: &ABIQ_SHIQ) -> bool {
        self.items.contains(abi)
    }
}

impl fmt::Display for ABQ_SHIQ {
...
impl ABQ_SHIQ {
    ...
    pub fn from_vec(name: &str, mut v: Vec<ABIQ_SHIQ>) -> ABQ_SHIQ {
        let mut abq = ABQ_SHIQ::new(name);
    ...
    pub fn sub_abox(&self, index: Vec<usize>, name: Option<&str>) -> Option<ABQ_SHIQ> {
        let name = match name {
            Option::None => "tmp",
            Some(s) => s,
        };

        let mut sub_abox = ABQ_SHIQ::new(name);
    ...
    pub fn is_inconsistent_detailed(&self, tb: &TB_SHIQ, verbose: bool) -> Vec<(TBI_SHIQ, Vec<ABIQ_SHIQ>)> {
        let tbis = tb.items();
        ...
        let mut contradictions: Vec<(TBI_SHIQ, Vec<ABIQ_SHIQ>)> = Vec::new();

        let mut combs_added_one: Vec<&ABIQ_SHIQ> = Vec::new();
        let mut combs_added_two: Vec<(&ABIQ_SHIQ, &ABIQ_SHIQ)> = Vec::new();
                ...
                if node_i == left {
                    ...
                    for j in 0..self_length {
                        ...
                        if abq_i.same_nominal(abq_j) && node_j == (&right_negated) {
                            ...
                            let mut to_add: Vec<ABIQ_SHIQ> = Vec::new();
    ...
    pub fn is_inconsistent(&self, tb: &TB_SHIQ, verbose: bool) -> bool {
        let tbis = tb.items();
        ...
        for tbi in tbis {
            ...
            let right_keeper: Node_SHIQ;
            let right_mod: &Node_SHIQ;
            ...
            if tbi.is_negative_inclusion() {
                ...
                right_mod = Node_SHIQ::child_old(Some(right)).unwrap();
            } else {
                if verbose {
    ...
    pub fn complete(&self, tbox: &TB_SHIQ, verbose: bool) -> ABQ_SHIQ {
        type T = TBI_SHIQ;
        type A = ABIQ_SHIQ;
        ...
        if self.items.len() == 0 {
            ...
            // keep the items
            let items: Arc<Mutex<VecDeque<ABIQ_SHIQ>>> = Arc::new(Mutex::new(VecDeque::new()));
            let items_temporal: Arc<Mutex<VecDeque<ABIQ_SHIQ>>> = Arc::new(Mutex::new(VecDeque::new()));
            ...
            let mut current_item: ABIQ_SHIQ; // at each iteration keeps the item to be treated
            ...
            */
            let rule_one: AbRule<T, A> = shiq_abox_rule_one;
            let rule_two: AbRule<T, A> = shiq_abox_rule_two;
            let rule_three: AbRule<T, A> = shiq_abox_rule_three;
                    ...
                    for index in 0..length {
                        ...
                        for rule_index in 0..number_of_rules {
                            ...
                            for tbi in tbox.items() {
                                ...
                                let new_item_vec3 =
                                    ABIQ_SHIQ::apply_rule(vec![&current_item, &item], vec![tbi], rule);
                                ...
                                for optional_vec in vec![&new_item_vec3] {
                                    ...
                                    if optional_vec.is_some() {
                                        let mut abis_to_add: Vec<ABIQ_SHIQ> = Vec::new();
            ...
            let mut new_abq = ABQ_SHIQ::new(&new_name);
            {
                let mut items = items.lock().unwrap();
