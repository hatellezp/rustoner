...
// internal imports
use crate::shiq::helpers_and_utilities::{
    complete_helper_add_if_necessary_general, complete_helper_dump_from_mutex_temporal_to_current2,
};
use crate::shiq::rule::{
    shiq_rule_eight, shiq_rule_five, shiq_rule_four, shiq_rule_one, shiq_rule_seven,
    shiq_rule_six, shiq_rule_three, shiq_rule_two, shiq_rule_zero,
};
...
impl TBox for TB_SHIQ {
    type TbiItem = TBI_SHIQ;

    fn len(&self) -> usize {
        self.length
    }

    fn add(&mut self, tbi: TBI_SHIQ) -> bool {
        /*
    ...
    fn items(&self) -> &Vec<TBI_SHIQ> {
        &(self.items)
    }

    fn get(&self, index: usize) -> Option<&TBI_SHIQ> {
    ...
    fn contains(&self, tbi: &TBI_SHIQ) -> bool {
...
impl TB_SHIQ {
    pub fn new() -> TB_SHIQ {
        let items: Vec<TBI_SHIQ> = Vec::new();
        TB_SHIQ {
            items,
            length: 0,
            completed: false,
    ...
    pub fn new_from_iter<I>(it: I) -> TB_SHIQ
    where
        I: Iterator<Item =TBI_SHIQ>,
    {
        let mut tb = TB_SHIQ::new();
    ...
    pub fn remove_trivial(&mut self) {
        if !self.items.is_empty() {
            let mut new_items: Vec<TBI_SHIQ> = Vec::new();
    ...
    pub fn negative_inclusions(&self) -> Vec<&TBI_SHIQ> {
        let mut neg_tbi: Vec<&TBI_SHIQ> = Vec::new();
    ...
    pub fn complete(&self, deduction_tree: bool, verbose: bool) -> TB_SHIQ {
        ...
        if self.items.len() == 0 {
            ...
            // keep the items
            let items: Arc<Mutex<VecDeque<TBI_SHIQ>>> = Arc::new(Mutex::new(VecDeque::new()));
            let items_temporal: Arc<Mutex<VecDeque<TBI_SHIQ>>> = Arc::new(Mutex::new(VecDeque::new()));
            ...
            let mut current_item: TBI_SHIQ; // at each iteration keeps the item to be treated
            ...
            */
            let rule_zero: TbRule<T> = shiq_rule_zero;
            let rule_one: TbRule<T> = shiq_rule_one;
            let rule_two: TbRule<T> = shiq_rule_two;
            let rule_three: TbRule<T> = shiq_rule_three;
            let rule_four: TbRule<T> = shiq_rule_four;
            let rule_five: TbRule<T> = shiq_rule_five;
            let rule_six: TbRule<T> = shiq_rule_six;
            let rule_seven: TbRule<T> = shiq_rule_seven;
            let rule_eight: TbRule<T> = shiq_rule_eight;
            ...
            // X=>Top and Bottom=>Y
            {
                // first put everything in 'items_temporal'
                {
                    ...
                    for index in 0..length {
                        ...
                        let new_item_vec = TBI_SHIQ::apply_rule(vec![item], &rule_zero, deduction_tree);
            ...
            // A=>notB then B=>notA
            {
                // first put everything in 'items_temporal'
                {
                    ...
                    for index in 0..length {
                        ...
                        let new_item_vec = TBI_SHIQ::apply_rule(vec![item], &rule_one, deduction_tree);
                 ...
                 */
                {
                    ...
                    for index in 0..length {
                        ...
                        for rule_index in 0..number_of_rules {
                            ...
                            let new_item_vec3 =
                                TBI_SHIQ::apply_rule(vec![&current_item, &item], rule, deduction_tree);

                            for optional_vec in vec![&new_item_vec3] {
                                // if the rule succeeded

                                if optional_vec.is_some() {
                                    let mut tbis_to_add: Vec<TBI_SHIQ> = Vec::new();
                                    let iterator = optional_vec.as_ref().unwrap();
                                    // try to apply rule zero and one
                                    for tbi in iterator {
                                        // added deduction tree
                                        let zero_tbi =
                                            TBI_SHIQ::apply_rule(vec![tbi], &rule_zero, deduction_tree);
                                        let one_tbi =
                                            TBI_SHIQ::apply_rule(vec![tbi], &rule_one, deduction_tree);
