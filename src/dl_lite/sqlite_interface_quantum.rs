...
use crate::dl_lite::string_formatter_quantum::string_to_abiq;
...
pub fn get_table_names(conn: &Connection, verbose: bool) -> Result<Vec<String>> {
    ...
    match smt_res {
        Err(e) => {
            if verbose {
                println!(" -- sqlite_interface_quantum::get_table_names: an error occurred: {}", &e);
            }
            Err(e)
        }
        Ok(mut smt) => {
...
pub fn drop_tables_from_database(conn: &Connection, tables_to_drop: Vec<&str>, verbose: bool) {
    ...
    for table in tables_to_drop {
        ...
        if table.contains("_abox_concept") || table.contains("_abox_role") {
            ...
            if verbose {
                println!(" -- sqlite_interface_quantum::drop_tables_from_database: query: {}\nreturned: {:?}", query, &res);
            }
        }
    }
}
